    ...
    m_RenderID(0), m_FilePath(filepath), m_Buffer(nullptr), m_Width(0), m_Height(0), m_BPP(0)
{
    ...
    std::future<unsigned char *> futureBuffer = std::async(std::launch::async, [this, filepath](){
        return stbi_load(filepath.c_str(), &m_Width, &m_Height, &m_BPP, 4);
    }); 
